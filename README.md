...
1주차. W, A, S, D키로 이동을 할수 있다.(메인 캐릭터 이동 구현)

2주차. 무기를 장착 할수 있으며 한번 장착한 무기는 바꿀 수 없다, 마우스 왼쪽 버튼 클릭으로 공격을, 마우스 오른쪽 버튼 클릭으로 방어를 할수 있다, 스페이스바 키로 회피(구르기)를 한다(메인 캐릭터 무기 장착 및 공격&방어 구현)

3주차. Q, E 키로 무기에 연동된 공격 스킬을 사용한다, 왼쪽Ctrl 키로 마법 스킬을 사용한다, R버튼으로 포션을 사용 하며 사용시 HP가 회복 된다.(메인 캐릭터 스킬 구현)

4주차. 플레이어가 보스룸에 들어 올때까지 보스룸 가운데에 대기한다, 플레이어와 보스 사이의 거리가 돌진거리보다 멀 경우 걸어서 이동한다, 플레이어가 공격 범위 안에 들어오면 약 공격 or 강 공격을 사용 한다.(1번 보스 스폰 및 일반 공격, 이동 구현)

5주차. 정확한 타이밍에 방어시 패링에 성공 한다, 플레이어와의 거리가 돌진거리와 같거나 작으면 돌진공격 or 마법 공격으로 견제를 한다(패링 구현 및 보스 공통 패턴 구현)

6주차. 돌진 공격후 약 공격 or 강 공격으로 연계된다(연계 공격 패턴 구현 및 기타 수정사항 수정 하기)
