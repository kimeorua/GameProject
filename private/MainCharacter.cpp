...
{
	if ((!IsRolling) && SP - RollingSP >= 0) // 현제 구르기 중인지 판단 아니라면 실행
	{
...
void AMainCharacter::QSkillActive()
{
	...
	if (!ActivateSkill && QSkillMontage != nullptr && IsHealing == false && !IsBlock) // 공격 중X, Q스킬이 설정 O, 포션 사용X
	{
...
{
	if (::IsValid(MainWeapon))	//매인캐릭터의 무기가 설정 되어 있으면 -> 검&방패 모드 or 대검모드
	{
		auto AnimInstance = Cast<UMainAnimInstance>(GetMesh()->GetAnimInstance());
